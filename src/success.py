...
TOOLS				= OrderedDict([
							('Object',				OrderedDict([	('reg', 'O'),		('tex',r'\textbf{O}'),		('color', (1.000, 0.000, 0.000)), ('marker', (3,0,0)),		('plt-loc', 0)	])), # Red
							('ObjectTime',			OrderedDict([	('reg', 'OT'),		('tex',r'\textbf{OT}'),		('color', (1.000, 0.500, 0.500)), ('marker', (3,0,180)),	('plt-loc', 1)	])), # Pink
							('Action',				OrderedDict([	('reg', 'A'),		('tex',r'\textbf{A}'),		('color', (0.000, 0.500, 0.000)), ('marker', (4,0,0)),		('plt-loc', 2)	])), # Green
							('ActionTime',			OrderedDict([	('reg', 'AT'),		('tex',r'\textbf{AT}'),		('color', (0.500, 1.000, 0.500)), ('marker', (4,0,45)),		('plt-loc', 3)	])), # Light Green
							('ActionObject',		OrderedDict([	('reg', 'AO'),		('tex',r'\textbf{AO}'),		('color', (0.000, 0.000, 1.000)), ('marker', (5,0,0)),		('plt-loc', 4)	])), # Blue
							('ActionObjectTime',	OrderedDict([	('reg', 'AOT'),		('tex',r'\textbf{AOT}'),	('color', (0.000, 0.750, 1.000)), ('marker', (5,0,180)),	('plt-loc', 5)	])), # Light Blue
							('CoalitionSimilarity',	OrderedDict([	('reg', 'CS'),		('tex','CS'),				('color', (0.500, 0.000, 0.500)), ('marker', (6,0,0)),		('plt-loc', 6)	])), # Purple
							('CoalitionAssistance',	OrderedDict([	('reg', 'CA'),		('tex','CA'),				('color', (1.000, 0.500, 1.000)), ('marker', (6,0,30)),		('plt-loc', 7)	])), # Light Purple
							('CFP',					OrderedDict([	('reg', 'CFP'),		('tex','CFP'),				('color', (0.000, 0.000, 0.000)), ('marker', (7,0,0)),		('plt-loc', 8)	])), # Black
							('PA',					OrderedDict([	('reg', 'PA'),		('tex','PA'),				('color', (0.500, 0.500, 0.500)), ('marker', (7,0,180)),	('plt-loc', 9)	])), # Grey
					])
...
def generate_plot(data):
	...
	num_charts = 9 if domain == 'first_response' else 10
	f, axes = plt.subplots(1,num_charts, sharex='all', sharey='all', squeeze=True)
	# plt.subplots_adjust(hspace=0.20, wspace=-0.30, bottom=0.15, top=0.88, left=0.00, right=1.00)
	plt.subplots_adjust(wspace=0.05, bottom=0.15, top=0.95, left=0.04, right=1.00)
	plt.gcf().set_size_inches(4.5,0.95)
	...
	if fusion_ratio == '0.25':
		...
		axes[5].legend(patches, labels, loc='center', frameon=False, ncol=4, bbox_to_anchor=(0.0, 1.30), fontsize=FONT_SIZE)

	for t in data:

		# Plot location indexes
		plt_loc = TOOLS[t]['plt-loc']
